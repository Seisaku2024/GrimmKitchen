...
namespace ArborEditor.BehaviourTree.UIElements
{
	...
	internal sealed class ChildLinkSlotElement : NodeLinkSlotElement
	{
		...
		protected override bool HasBranch(int branchID)
		{
			NodeBranch branch = _RootNode.GetChildBranch();
			return branch != null && branch.branchID == branchID;
		}
		...
		protected override void OnChangedPosition()
		{
			...
			NodeBranch branch = _RootNode.GetChildBranch();
			if (branch != null 
				&& graphEditor.TryGetNodeBranchElement(branch.branchID, out var nodeBranchElement))
			{
				...
				if (nodeBranchElement.bezier.SetStartPoint(startPosition, startControl))
				{
					graphEditor.Repaint();
				}
			}
