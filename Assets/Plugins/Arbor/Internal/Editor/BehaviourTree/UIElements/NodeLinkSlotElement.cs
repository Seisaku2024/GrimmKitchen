	...
	internal abstract class NodeLinkSlotElement : VisualElement
	{
		...
		protected virtual void OnAttachToPanel(AttachToPanelEvent e)
		{
			...
			if (graphEditor != null)
			{
				...
				graphEditor.onAddBranchElement -= OnAddBranchElement;
				graphEditor.onAddBranchElement += OnAddBranchElement;
			}
		...
		void OnAddBranchElement(int branchID)
		{
		...
		protected abstract bool HasBranch(int branchID);
		...
		void Connect(TreeNodeBase parentNode, TreeNodeBase childNode, Bezier2D bezier)
		{
			...
			if (branch != null)
			{
				graphEditor.UpdateNodeBranchies();

				if (graphEditor.TryGetNodeBranchElement(branch.branchID, out var nodeBranchElement))
				{
					nodeBranchElement.bezier.SetStartPoint(bezier.startPosition, bezier.startControl);
					nodeBranchElement.bezier.SetEndPoint(bezier.endPosition, bezier.endControl);
				}
		...
		sealed class ConnectManipulator : DragOnGraphManipulator
		{
				...
				if (graphEditor != null && _Dragging)
				{
					...
					if (hoverNode != null)
					{
						if (!slotElement.IsConnected(hoverID))
						{
							...
							{
								slotElement.Connect(parentNode, childNode, _DragBezier);
							}
						}
					}
					else
			...
			void OnSelectAction(Vector2 pos, System.Type classType)
			{
				...
				slotElement.Connect(slotElement.nodeEditor.treeNode, actionNode, _DragBezier);
