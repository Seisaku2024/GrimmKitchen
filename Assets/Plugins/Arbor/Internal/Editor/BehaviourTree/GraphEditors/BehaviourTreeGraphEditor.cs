...
namespace ArborEditor.BehaviourTree
{
	...
	public sealed class BehaviourTreeGraphEditor : NodeGraphEditor
	{
		...
		internal event System.Action<int> onAddBranchElement;

		internal void UpdateNodeBranchies()
		{
			...
			for (int branchIndex = 0; branchIndex < branchCount; branchIndex++)
			{
				...
				if (!_NodeBranchElements.TryGetValue(branch.branchID, out var branchElement))
				{
					...
					onAddBranchElement?.Invoke(branch.branchID);
				}
		...
		internal bool TryGetNodeBranchElement(int branchId, out NodeBranchElement nodeBranchElement)
		{
			return _NodeBranchElements.TryGetValue(branchId, out nodeBranchElement);
		}
		...
		void UpdateMinimapNodeBranch(BezierElement branchElement, NodeBranch branch)
		{
			...
			if (TryGetNodeBranchElement(branch.branchID, out var nodeBranchElement))
			{
				Bezier2D bezier = minimapView.GraphToMinimap(nodeBranchElement.bezier);
