...
#if ARBOR_DOC_JA
	/// <summary>
	/// フィールドに対してジェネリックの引数に指定されている型を要素としたIList&lt;T&gt;のみに制限する属性。
	/// </summary>
	/// <remarks>
	/// 利用可能なクラス : <br/>
	/// <list type="bullet">
...
#endif
	[AttributeUsage(AttributeTargets.Field, AllowMultiple = false)]
	public sealed class ClassGenericArgumentListAttribute : ClassTypeConstraintAttribute
	{
		private int _GenericArgumentIndex = 0;
		...
		/// <param name="genericArgumentIndex">Index of generic argument</param>
#endif
		public ClassGenericArgumentListAttribute(int genericArgumentIndex)
		{
			_GenericArgumentIndex = genericArgumentIndex;
		}
		...
		public override Type GetBaseType(FieldInfo fieldInfo)
		{
			...
			return typeof(System.Collections.Generic.IList<>).MakeGenericType(constraintType);
		}
		...
		public override bool IsConstraintSatisfied(Type type, FieldInfo fieldInfo)
		{
			var baseType = GetBaseType(fieldInfo);
			if (baseType == null)
			{
				return false;
			}
