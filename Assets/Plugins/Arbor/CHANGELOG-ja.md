...
- Universal Windows Plalformでの動作が正常に行われない不具合を修正。
...
- Unity 6000.5.0f1の動作サポート追加

### 修正

- Unity 6.4以降でArbor Editorウィンドウのスクリーンショット機能を使用すると、ウィンドウの終了処理時に例外が発生する不具合を修正。
...
- Unity 6000.4.0f1の動作サポート追加

### 変更

- エディタ上でプレイ中にLoadSceneスクリプトかRestartSceneスクリプトでシーン読み込みする際、未登録シーンの場合はEditorSceneManagerを介して読み込むように変更(Unity内部動作変更への対応)

### 修正

- ライブ追跡のトグルなどが表示されない不具合を修正
- コンパイル直後に言語パックの読み込みが遅延する不具合を修正
- サンプルのExampleSelectorから各シーンが読み込めない不具合を修正
...
- Unity6000.3以降で挙動追加メニューの検索バーに入力フォーカスがあるように見えても入力が反映されない不具合を修正。
- 新規インポート時に各種エディタウィンドウのテキスト文言が正常表示されない不具合を修正。
...
- Unityの最小対応バージョンをUnity6000.0に変更。
- インポート先フォルダをPackagesに変更。
- サンプルのRender PipelineをUniversal Render Pipelineに変更。
- サンプルをPackage ManagerのSamplesからインポートするように変更。

### 廃止

- Arbor.Poolのオブジェクトプール型を非推奨に変更。  
  代わりにUnityEngine.Poolを使用してください。

### 修正

- Mac環境でのノードなどのリネーム時にIME変換が正常に動作しない不具合を修正。
...
- Unity 6000.3.0f1の動作サポート追加

### 修正

- Unity 2022で型選択ポップアップ表示中に再度ドロップダウンフィールドをクリックするとフリーズする不具合を修正
- Unity 6000.1以降で接続線にマウスオーバーすると表示が消える不具合を修正。
