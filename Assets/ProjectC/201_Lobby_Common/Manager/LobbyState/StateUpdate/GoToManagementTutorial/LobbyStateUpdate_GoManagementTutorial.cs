...
public class LobbyStateUpdate_GoManagementTutorial : BaseLobbyStateUpdate
{
    ...
    private async UniTask CreateWindow()
    {
        ...
        try
        {
            ...
            m_challengeWindowController = Instantiate(m_challengeWindowController);

            // 処理を行う
            await m_createWindowController.CreateWindow<BaseWindow>();
            cancelToken.ThrowIfCancellationRequested();

            await m_challengeWindowController.CreateWindow<BaseWindow>();
            cancelToken.ThrowIfCancellationRequested();

            var data = StoryProgressManager.instance.GetStoryProgressData(StoryProgressType.CompleteManagementTutrial);
            if(data)
            {
                data.SetFinish();
            }
    ...
    void DestoryControllerWindow()
    {
        if (m_createWindowController != null)
        {
            ...
            Destroy(m_challengeWindowController.gameObject);
        }
    }
