...
/// </summary>
[AddComponentMenu("Cinemachine/LockOnCinemachineCollider")]
[SaveDuringPlay]
[ExecuteAlways]
[DisallowMultipleComponent]
public class LockOnCinemachineCollider : CinemachineExtension
{
    ...
    public List<List<Vector3>> DebugPaths
    {
        get
        {
            ...
            var extraStates = new List<VcamExtraState>();
            GetAllExtraStates(extraStates);
        ...
        CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
    {
        ...
        if (stage == CinemachineCore.Stage.Body)
        {
            ...
            if (m_AvoidObstacles)
            {
                ...
                {
                    Vector3 correctedPosition = state.RawPosition + state.PositionCorrection;
                    Vector3 pos = correctedPosition + displacement;
                ...
                var cameraPos = state.RawPosition + state.PositionCorrection + displacement;
                //displacement += RespectCameraRadius(
                //    cameraPos, state.HasLookAt ? Player.position : cameraPos);
                displacement += RespectCameraRadius(cameraPos, Player.position);
        ...
        if (stage == CinemachineCore.Stage.Aim)
        {
            ...
            {
                float distance = Vector3.Magnitude(Player.position - (state.RawPosition + state.PositionCorrection));
    ...
    Vector3 PreserveLineOfSight(ref CameraState state, ref VcamExtraState extra)
    {
        ...
        {
            Vector3 cameraPos = state.RawPosition + state.PositionCorrection;
    ...
    bool CheckForTargetObstructions(CameraState state)
    {
        ...
        {
            Vector3 lookAtPos = Player.position;
            Vector3 pos = state.RawPosition + state.PositionCorrection;
    ...
    bool IsTargetOffscreen(CameraState state)
    {
        ...
        {
            Vector3 cameraPos = state.RawPosition + state.PositionCorrection;
            Vector3 dir = Player.position - cameraPos;
            ...
            else
            {
                ...
                float verticalAngle = Vector3.Angle(
                    Vector3.ProjectOnPlane(dir, Vector3.right),
                    Vector3.forward);

                if (verticalAngle > fov)
                    return true;

                float horizontalFov = Mathf.Rad2Deg * Mathf.Atan(
                    Mathf.Tan(fov * Mathf.Deg2Rad) * state.Lens.Aspect);
...
///// report this as a field in the camera State.
///// </summary>
//[AddComponentMenu("Cinemachine/ChinemacineCollider")]
//[SaveDuringPlay]
//[ExecuteAlways]
//[DisallowMultipleComponent]
//public class LockOnCinemachineCollider : CinemachineExtension
