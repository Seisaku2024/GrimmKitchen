...
public class CameraRevarseYPresenter : MonoBehaviour
{
    [SerializeField]
    ...
    private void Initialized()
    {
        ...
        // InvertYの値を取得
        bool? invertY = GetInvertValue(processors);
        if(invertY == null )
        {
        ...
        m_toggleElementController.SetValue(!invertY.Value);
        ChangeCameraRevarse();
    }
    ...
    // `invertY` の値を設定
    private string SetInvertValues(string input, /*bool invertX, */bool invertY)
    {
    ...
    {
        Match match = Regex.Match(input, $@"invertY=(true|false)");
        if (match.Success)
        {
